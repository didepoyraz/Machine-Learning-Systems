...
from tracestorm.trace_player import TracePlayer, play
...
class RAGServiceTester:
    def __init__(self, host: str = "localhost", port: int = 8000, output_dir: str = "results"):
        ...
        # For OpenAI API compatibility (required by TracePlayer)
        self.api_key = "dummy-key"  # TracePlayer requires an API key even if not used
    
    def prepare_requests(self, num_requests: int) -> List[Dict[str, Any]]:
        """
        Prepare a list of request payloads that are compatible with TracePlayer.
        Each request is formatted to look like an OpenAI API call but will be
        redirected to our RAG endpoint.
        """
        requests = []
        for i in range(num_requests):
            query = self.queries[i % len(self.queries)]
            # Format the request in a way compatible with TracePlayer
            # but containing our RAG-specific data
            requests.append({
                "model": "rag-service",
    ...
    def run_trace_test(self, trace_config: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Running a trace test using the TracePlayer from trace_player.py
        """
        ...
        # Convert arrival times to milliseconds for TracePlayer
        trace_ms = [int(t * 1000) for t in request_profile.arrival_times]
        
        # Prepare request payloads
        requests = self.prepare_requests(len(trace_ms))
        
        # Create IPC queue for results
        result_queue = multiprocessing.Queue()
        
        # Adjust concurrency based on config
        concurrency = trace_config.get("max_concurrency", 20)
        
        # Create and start the TracePlayer process
        player_name = f"rag-test-{trace_config['rate_pattern']}"
        
        # The play function starts a TracePlayer in a subprocess
        process = multiprocessing.Process(
            target=play,
            args=(player_name, trace_ms, requests, self.base_url, self.api_key, result_queue)
        )
        process.start()
        ...
        results = []
        results_count = 0
        expected_count = len(trace_ms)
        
        print(f"Waiting for {expected_count} results...")
        while results_count < expected_count:
            try:
                # Get result with timeout to avoid blocking indefinitely
                name, timestamp, result = result_queue.get(timeout=30)
                
                # Process the result
                processed_result = {
                    "request_id": results_count,
                    "arrival_time": timestamp / 1000,  # Convert back to seconds
                    "client_time": (result["time_records"][-1] - result["time_records"][0]) 
                                   if result["time_records"] else 0,
                ...
                if results_count % 10 == 0:
                    print(f"Completed {results_count}/{expected_count} requests")
                    
            except multiprocessing.queues.Empty:
                print("Queue timeout. Checking if process is still alive...")
                if not process.is_alive():
                    print("TracePlayer process has terminated.")
                    break
    ...
    def analyze_results(self, results: List[Dict[str, Any]], test_name: str) -> Dict[str, Any]:
        ...
        if successful_results and "token_count" in successful_results[0]:
    ...
    def run_test_patterns(self):
        """
        Defines several test patterns with specific configurations.
        """
